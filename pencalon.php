...
<div class="panel panel-default">
    <div class="panel-heading">
        <form class="form-inline">
            ...
            <div class="form-group <?= ($_SESSION['akses'] == 'admin') ? '' : 'hidden' ?>">
                <a class="btn btn-primary" href="?m=pencalon_tambah"><span class="glyphicon glyphicon-plus"></span>
                    Tambah</a>
            </div>
            <div class="form-group">
                <a class="btn btn-default" href="cetak.php?m=pencalon&q=<?= $_GET['q'] ?>" target="_blank"><span
    ...
    <table class="table table-bordered table-hover table-striped">
        ...
        <tr>
            ...
            <td><img src="gambar/<?= $row->gambar ?>" style="height: 200px;" /></td>
            <td><?= $row->nama_pencalon ?></td>
            <td><?= $row->keterangan ?></td>
            <td><?= number_format($row->total) ?></td>
            <td class=" <?= ($_SESSION['akses'] == 'admin') ? '' : 'hidden' ?>">
                <a class="btn btn-xs btn-warning" href="?m=pencalon_ubah&ID=<?= $row->id_pencalon ?>"><span
                        class="glyphicon glyphicon-edit"></span></a>
                <a class="btn btn-xs btn-danger" href="aksi.php?act=pencalon_hapus&ID=<?= $row->id_pencalon ?>"
